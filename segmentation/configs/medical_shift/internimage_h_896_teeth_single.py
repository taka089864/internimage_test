...
# InternImage-H (1.12B params, 3,566 GFLOPs)
_base_ = [
...
pretrained = 'checkpoint/upernet_internimage_h_896_160k_ade20k.pth'
...
model = dict(
    ...
    backbone=dict(
        ...
        channels=320,  # H: 320 (XL: 192, L: 160)
        depths=[6, 6, 32, 6],  # H: [6, 6, 32, 6] (XL: [5, 5, 24, 5])
        groups=[10, 20, 40, 80],  # H: [10, 20, 40, 80] (XL: [12, 24, 48, 96])
        mlp_ratio=4.,
        drop_path_rate=0.5,  # H: 0.5 (XL: 0.4)
        norm_layer='LN',
        layer_scale=None,  # H: None (XL: 1.0)
        offset_scale=1.0,  # H: 1.0 (XL: 2.0)
        post_norm=False,  # H: False (XL: True)
        # InternImage-H/G専用パラメータ
        dw_kernel_size=5,
        res_post_norm=True,
        ...
        # True: Gradient checkpointing有効化（VRAM削減、速度低下あり）
        with_cp=True,
        # with_cp=False,
    ...
    decode_head=dict(
        ...
        in_channels=[320, 640, 1280, 2560],  # H (XL: [192, 384, 768, 1536])
        in_index=[0, 1, 2, 3],
        pool_scales=(1, 2, 3, 6),
        channels=128,
        dropout_ratio=0.1,
...
# ============================================================
# 注意: Hugeモデルは非常に大きいため、batch_sizeを調整（公式設定は1）
train_dataloader = dict(
    batch_size=10,  # H: 4 - OOMの場合は1に削減
    ...
    dataset=dict(
        ...
        pipeline=[
            ...
            dict(type='Resize', scale=(896, 896), keep_ratio=True),  # H: 896 (XL/L: 640)
            dict(type='RandomCrop', crop_size=(896, 896), cat_max_ratio=0.75),  # H: 896
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        pipeline=[
            ...
            dict(type='Resize', scale=(896, 896), keep_ratio=True),  # H: 896
            dict(type='Normalize', mean=[148.0188864091492, 148.0188864091492, 148.0188864091492],
                 std=[57.24381990728073, 57.24381990728073, 57.24381990728073], to_rgb=False),
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type='AdamW',
        lr=0.00002,  # H: 0.00002
        betas=(0.9, 0.999),
        weight_decay=0.05
    ),
    constructor='CustomLayerDecayOptimizerConstructor',
    paramwise_cfg=dict(
        num_layers=50,  # H: 50 (6+6+32+6) (XL: 39, L: 37)
        layer_decay_rate=0.95,  # H: 0.95 (XL/L: 0.94)
        depths=[6, 6, 32, 6],  # H: [6, 6, 32, 6]
        offset_lr_scale=1.0
    ),
