...
# InternImage-XL (368M params, 3,142 GFLOPs)
_base_ = [
...
pretrained = 'checkpoint/upernet_internimage_xl_640_160k_ade20k.pth'
...
model = dict(
    ...
    backbone=dict(
        ...
        channels=192,  # XL: 192 (L: 160)
        depths=[5, 5, 24, 5],  # XL: [5, 5, 24, 5] (L: [5, 5, 22, 5])
        groups=[12, 24, 48, 96],  # XL: [12, 24, 48, 96] (L: [10, 20, 40, 80])
        mlp_ratio=4.,
        drop_path_rate=0.4,  # XL: 0.4
        norm_layer='LN',
        layer_scale=1.0,
        offset_scale=2.0,  # XL: 2.0
        post_norm=True,
        with_cp=False,
        out_indices=(0, 1, 2, 3),
    ...
    decode_head=dict(
        ...
        in_channels=[192, 384, 768, 1536],  # XL (L: [160, 320, 640, 1280])
        in_index=[0, 1, 2, 3],
        pool_scales=(1, 2, 3, 6),
        channels=128,
        dropout_ratio=0.1,
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type='AdamW',
        lr=0.00002,  # XL: 0.00002
        betas=(0.9, 0.999),
        weight_decay=0.05
    ),
    constructor='CustomLayerDecayOptimizerConstructor',
    paramwise_cfg=dict(
        num_layers=39,  # XL: 39 (5+5+24+5) (L: 37)
        layer_decay_rate=0.94,  # XL: 0.94
        depths=[5, 5, 24, 5],  # XL: [5, 5, 24, 5]
        offset_lr_scale=1.0
    ),
